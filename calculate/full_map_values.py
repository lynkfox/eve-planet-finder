...
class CalculatedMapData():
    ...
    def PlanetIdsByTypeIds(self)->Dict[int, List[int]]:
        return {planet_type.Id: [planet.Id for planet in self.all_data.Planets if planet.Type_Id == planet_type.Id] for planet_type in self.all_data.Planet_Types}
    ...
    def AveragePlanetTypeBySystem(self)->Dict[int, float]:
        return {planet_type.Id: numpy.average([len([planet.Type_Id for planet in sys.GetPlanets() if planet.Type_Id == planet_type.Id])/len(sys.Planet_Ids) for sys in self.all_data.Systems]) for planet_type in self.all_data.Planet_Types}
      
    @cached_property
    def MeanPlanetTypeBySystem(self)->Dict[int, float]:
        return {planet_type.Id: numpy.mean([len([planet.Type_Id for planet in sys.GetPlanets() if planet.Type_Id == planet_type.Id])/len(sys.Planet_Ids) for sys in self.all_data.Systems]) for planet_type in self.all_data.Planet_Types}
