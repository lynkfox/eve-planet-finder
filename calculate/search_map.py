...
class WeightCalculator():
    ...
    def _checkNextSystem(self, current_sys: System, previous_system_id: int, current_jumps:int, current_matched_values: set) -> Tuple[bool, set]:
        ...
        """
        logging_prefix = f"Weighting [{current_sys.Name}#{current_sys.Id}@Jump-{current_jumps} From {previous_system_id}]"
        ...
        if current_sys.Id in self._systemIdsAlreadySearched.keys() and self._systemIdsAlreadySearched[current_sys.Id] < current_jumps:
        ...
        weight, weight_values, matched_values = self.WeightFactors.DetermineSystemWeight(
            current_sys,
            current_jumps)
        ...
        if new_matched_values == self.MustFindTargets:
            ...
            self._add_log(f"{logging_prefix} Is Final System in Chain, still not all required values found. Returning False.")
            return False
        ...
        for new_target_system in current_sys.GetLinkedSystems(cache=self.CACHE_EXPENSIVE_CALLS):
            ...
            did_chain_find_everything = self._checkNextSystem(new_target_system, current_sys.Id, current_jumps+1, new_matched_values)
            ...
            if not did_chain_find_everything and self._results.get(new_target_system.Id, None) is not None:
                ...
                self._add_log(f"{logging_prefix} No children of this chain meet all targets. Deleting child #{new_target_system.Id}")
